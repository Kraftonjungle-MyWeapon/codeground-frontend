                  ...
                  <ScrollArea className="h-full">
                    <div className="space-y-3 pr-4">
                      {waitingRooms.map((room) => (
                        <div key={room.id} className="flex items-center justify-between p-4 bg-black/20 rounded-lg hover:bg-black/30 transition-colors">
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-1">
                              <Crown className="h-4 w-4 text-yellow-400" />
                              <span className="text-white font-medium">{room.title}</span>
                              <span className={`px-2 py-1 text-xs rounded-full ${
                                room.difficulty === '초급' ? 'bg-green-500/20 text-green-400' :
                                room.difficulty === '중급' ? 'bg-yellow-500/20 text-yellow-400' :
                                room.difficulty === '고급' ? 'bg-orange-500/20 text-orange-400' :
                            ...
                            <p className="text-xs text-gray-500">호스트: {room.hostName} ({room.hostTier})</p>
                          </div>
                  ...
                  <ScrollArea className="h-full">
                    <div className="space-y-2 pr-4">
                      {topRanking.map((player) => {
                        const { tier: playerTier, lp: playerLp } = parseTotalScore(player.totalScore);
                        return (
                          <div key={player.rank} className="flex items-center justify-between p-3 bg-black/20 rounded-lg hover:bg-black/30 transition-colors">
                            <div className="flex items-center space-x-3">
                              <div className="flex items-center justify-center w-8 h-8">
                                {player.rank === 1 && <Crown className="h-5 w-5 text-yellow-400" />}
                                {player.rank === 2 && <Trophy className="h-5 w-5 text-gray-300" />}
                                {player.rank === 3 && <Star className="h-5 w-5 text-orange-400" />}
                                {player.rank > 3 && <span className="text-white font-bold text-sm">#{player.rank}</span>}
                              </div>
                              <div>
