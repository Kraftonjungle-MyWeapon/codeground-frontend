        ...
        <div className="w-full max-w-4xl">
          <CyberCard className="text-center p-12 relative overflow-hidden border-red-500/30">
            ...
            <div className="relative z-10 space-y-8">
              <h1 className="text-4xl font-bold text-red-400 mb-12">티어 강등</h1>
              
              <div className="relative h-60 flex items-center justify-center">
                {/* 이전 티어 - 번개에 맞기 전까지 표시 */}
                <div className={`absolute transition-all duration-500 ${
                  animationPhase >= 3 ? 'opacity-0 scale-0 rotate-12' : 'opacity-100 scale-100'
                } ${
                  ...
                  <div className="flex flex-col items-center space-y-4 relative">
                    ...
                    {animationPhase >= 2 && animationPhase < 4 && (
                      <>
                        <Zap className="h-8 w-8 text-yellow-400 absolute -top-2 -left-8 animate-ping" 
                             style={{ animationDelay: '0s' }} />
                        <Zap className="h-6 w-6 text-yellow-400 absolute -top-4 right-6 animate-ping" 
                             style={{ animationDelay: '0.2s' }} />
                        <Zap className="h-10 w-10 text-yellow-400 absolute bottom-8 -left-6 animate-ping" 
                             style={{ animationDelay: '0.4s' }} />
                        <Zap className="h-7 w-7 text-yellow-400 absolute bottom-6 -right-8 animate-ping" 
                             style={{ animationDelay: '0.6s' }} />
                        <Zap className="h-9 w-9 text-yellow-400 absolute top-2 right-10 animate-ping" 
                ...
                <div className={`absolute transition-all duration-1000 ${
                  animationPhase >= 4 ? 'opacity-100 scale-100' : 'opacity-0 scale-50'
                }`}>
                  <div className="flex flex-col items-center space-y-4">
                    <Crown className={`h-28 w-28 ${newTier.color} opacity-70`} />
